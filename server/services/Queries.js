...
class QueryService {
  ...
  async Get700TCraneData() {
    ...
    .ExecuteStoredProcedure(`Get700TCraneData`,[], this.callback).then(function(response){
  ...
  async Crane700T_GetCraneTrendsSQL(category){
    return await this.sqlInterface
    .ExecuteStoredProcedure(`Get700TCraneLiftTrend`,[{name:'category', type:sql.VarChar(100), value: category}], this.callback).then(function(response){
        //console.log(response);
        return response
    });
  }
